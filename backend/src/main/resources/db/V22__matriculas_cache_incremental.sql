...
BEGIN
    ...
    USING (
        SELECT
            ...
            :NEW.id matricula_id,
            :NEW.dt_inscricao matricula_data,
            :NEW.situacao matricula_situacao,
            :NEW.frequencia matricula_frequencia,
            :NEW.media_final matricula_media_final,
            pr.id prova_id,
            pr.codigo prova_codigo,
            pr.peso prova_peso,
            DBMS_LOB.SUBSTR(pr.conteudo, 500, 1) prova_conteudo,
            r.id resultado_id,
            r.nota resultado_nota,
            r.presente resultado_presente,
            r.data_realizacao resultado_data_realizacao,
            r.duracao_min resultado_duracao_min
          FROM turmas t
          LEFT JOIN alunos a ON a.id = :NEW.aluno_id
          ...
          LEFT JOIN resultados_prova r ON r.matricula_id = :NEW.id AND r.prova_id = pr.id
         WHERE t.id = :NEW.turma_id
    ) src
    ON (cache.matricula_id = src.matricula_id AND NVL(cache.prova_id, -1) = NVL(src.prova_id, -1))
